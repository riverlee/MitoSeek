...
our ( %OPT,
     ...
     $MAP_MAKE, @MAP_ELEMENTS,  @IDEOGRAMS, $KARYOTYPE, 
     $GCIRCUM, $GSIZE_NOSCALE );
...
sub run {
    ...
    printdebug_group("conf","welcome to circos",$VERSION);
    ...
    for my $f ( qw(ideogram_spacing unit_parse unit_strip locate_file getrelpos_scaled_ideogram_start text_size is_counterclockwise debug_or_group seek_parameter unit_strip unit_validate getanglepos get_angle_pos)
	) {
	memoize($f);
    }
    ...
    for my $fontfile (map { locate_file(file=>$_, name=>"font [$_]") } values %{$CONF{fonts}}) {
	my $text = "abc";
	my @label_bounds = GD::Image->stringFT(fetch_color("black",$COLORS),
					       $fontfile,
					       10,
					       0, 0, 0, 
					       $text);
	...
	if(! $label_width || ! $label_height) {
	    fatal_error("font","no_ttf",$fontfile);
	}
    }
  ...
  for my $highlight_set ( make_list( $CONF{highlights}{highlight} ) ) {
	...
	locate_file( file => $highlight_set_param->{file}, name=>"highlight set" ),
	'highlight',
	{
  ...
  for my $ideogram (@IDEOGRAMS) {
	 ...
	 !seek_parameter( "show", $CONF{ideogram}) ) {
	  ...
	  my $font_name = get_font_name_from_key($font_key,$font_role);
	  
	  my $label  = $KARYOTYPE->{$chr}{chr}{label};
	  $label    .= $tag if $tag ne $chr && $tag !~ /__/ && fetch_conf("ideogram","label_with_tag");
	  ...
	  0&&draw_text(
	      ...
	      svgxy => [
		  getxypos(
		      $textangle + $offset_angle / $CONF{svg_font_scale},
		      ($CONF{ideogram}{label_parallel}||0) * $offset_radius + $DIMS->{ideogram}{ $tag }{label}{radius}
		  )
	      ],
...
for my $ideogram (@IDEOGRAMS) {
  ...
  #                                         [link1,link2,link3,...]
	...
	printdebug_group("summary","drawing link track",$linkname);

	if ( ref( $CONF{links}{link}{$linkname} ) eq 'ARRAY' ) {
	    fatal_error("links","duplicate_names",$linkname);
	}
	...
	next
	    ...
	    || seek_parameter( "show", @param_path );
	$dataset->{data} = read_data_file(
	    locate_file( file => $link_param->{file}, name=>"link [$linkname]" ),
	    'link',
	    {
		addset       => 1,
		groupby      => 'id',
	...
	for my $datum ( @{ $dataset->{data} } ) {
	    unless ( @{ $datum->{data} } > 1 ) {
		fatal_error("link","single_entry",$linkname,$datum->{data}[0]{id});
	    }
	}
	...
	if ($pass) {
	  ...
	  for my $rulekey ( keys %$ruleparam ) {
	    ...
	    printdebug_group("rule","link rule parameter",$rulekey,"root",$rulekey_root,"num",$rulekey_number,"value",$value);
	...
	if ($pass) {
	    if ( match_string($flow,"restart") && !$rule->{restart} ) {
		$rule->{restart} = 1;
		goto RULES;
	    }
	    last unless match_string($flow,"continue");
	}
      }
    }
  }
  ...
  my $dataset;
	...
	my $linkradius =
	    unit_parse( seek_parameter( "radius", @link_param_path ) || 0) 
	    +
	    unit_parse( seek_parameter( "offset", @link_param_path ) || 0);
	...
	for my $i ( 1 .. @{ $link->{data} } - 1 ) {
	    ...
	    my $linkz = seek_parameter( "z", @i_link_param_path ) || 0;
	    next unless (! $linkz && ! $targetz) || ($linkz == $targetz);
	    
	    printdebug_group("layer",
			     "drawing z",$targetz,
			     "links",$linkset->{param}{name}) if ! $link_report_seen->{$targetz}{$linkset->{param}{name}}++;
	  ...
	  my ( $radius1, $radius2 );
		...
		$radius1 =  
		    unit_parse( seek_parameter( "radius1", @i_link_param_path ), $ideogram1 ) 
		    +
		    unit_parse( seek_parameter( "offset", @link_param_path ) || 0 , $ideogram1) ;
	    } else {
		$radius1 = 
		    unit_parse( seek_parameter( "radius", [ $link->{data}[ $i - 1 ], @link_param_path ] ) || 0, $ideogram1 ) 
		    +
		    unit_parse( seek_parameter( "offset", @link_param_path ) || 0, $ideogram1 );
	    }
	  ...
	  if ( seek_parameter( "radius2", @i_link_param_path ) ) {
	      $radius2 =  unit_parse(
		  seek_parameter( "radius2", @i_link_param_path ), $ideogram2 ) 
		  +
		  (unit_parse(seek_parameter( "offset", @link_param_path ),$ideogram2 ) || 0);
	  } else {
	      $radius2 = unit_parse(
		  seek_parameter(
		      "radius",
	    ...
	    #printinfo("link",$radius1,$radius2);
	    if ( seek_parameter( "ribbon", @i_link_param_path ) ) {
		
		my ( $start1, $end1 ) = (
		    max(
			(
			 $link->{data}[ $i - 1 ]{param}{start} || $link->{data}[ $i - 1 ]{data}{set}->min
			),
			$ideogram1->{set}->min
		    ),
		    min(
			(
			 $link->{data}[ $i - 1 ]{param}{end} || $link->{data}[ $i - 1 ]{data}{set}->max
			),
			$ideogram1->{set}->max
		    )
		    );
	    ...
	    #printinfo("ribbon",$url);
	      #printinfo("ribbon",$radius1,$radius2);
	    ribbon(
		mapoptions => {url=> $url},
		image     => $IM,
		start1    => $start1,
	    ...
	    printdebug_group("bezier", "beziercontrols",
			     int(@bezier_control_points), @bezier_control_points );
		...
		$getu->( @bezier_control_points[ 0 .. 5 ] );
		  ...
		  unit_strip(seek_parameter("thickness", @i_link_param_path),"p"),
	...
	printdebug_group("summary","reading data for",$type,"track",$plot->{file});
	
	if ( $type eq "text" ) {
	    $dataset->{data} = read_data_file(
		locate_file( file => seek_parameter("file",@param_path), name=>"text track data file"),
		"text",
		{
		    addset => 0,
		    record_limit => seek_parameter( "record_limit", @param_path )
	    ...
	    $dataset->{data} = read_data_file(
		locate_file( file => seek_parameter("file",@param_path), name=>"highlight data file"),
		"highlight",
		{
		    addset => 0,
		    record_limit =>
	    ...
	    $dataset->{data} = read_data_file(
		locate_file( file => seek_parameter("file",@param_path), name=>"tile data file"),
		"tile",
		{
		    addset => 0,
		    record_limit =>
	    ...
	    $dataset->{data} = read_data_file(
		locate_file( file => seek_parameter("file",@param_path), name=>"connector data file"),
		"connector",
		{
		    addset => 0,
		    record_limit => seek_parameter( "record_limit", @param_path )
	    ...
	    $dataset->{data} = read_data_file(
		locate_file( file => seek_parameter("file",@param_path),name=>"histogram data file"),
		"plot",
		{
		    addset => 0,
	    ...
	    $dataset->{data} = read_data_file(
		locate_file( file => seek_parameter("file",@param_path), name=>"track data file"),
		"plot",
		{
		    addset   => 0,
		    skip_run => seek_parameter( "skip_run", @param_path ),
	...
	for my $dataset ( make_list( $data->{plots}{dataset} ) ) {
	    ...
	    if(defined seek_parameter("z",@param_path) && defined $targetz) {
	    ...
	    $plot_min = 0 if ! defined $plot_min;
	    $plot_max = 0 if ! defined $plot_max;
	  ...
	  my @label_data = sort {
	      ...
	      #	  || 
	      ( ($a->{data}[0]{data}{angle}||0) <=> ($b->{data}[0]{data}{angle}||0) ) } @{ $dataset->{data} };
	  ...
	  do {
	    ...
	    REFINE:
		  ...
		  if ($data_point_prev) {
			  ...
			  ( $data_point->{angle_new}, $data_point_prev->{angle_new} ) = 
			      ( $data_point_prev->{angle_new}, $data_point->{angle_new} );
	  ...
	  stop_timer("track_text_place");
		...
		$dataset->{data} = [ sort 
				     { ($b->{data}[0]{param}{z}||0) <=> ($a->{data}[0]{param}{z}||0) } 
				     @{$dataset->{data}} ];
	    }
	...
	if ( seek_parameter( "background", @param_path ) ) {
      ...
      my $sort_funcs = {
			text => sub { ($b->{data}[0]{data}{w}||0) <=> ($a->{data}[0]{data}{w}||0) },
			default => sub {
			    ( ($a->{data}[0]{param}{z}||0) <=> ($b->{data}[0]{param}{z}||0) )
	...
	#
	  ...
	  if ( $plot_type ne "connector" ) {
	      ...
	      next unless get_ideogram_by_idx($ideogram_idx)->{set}->member( $data_point->{start} )
	  ...
	  if ( $plot_type eq "text" ) {
		  ...
		  printdebug_group("textplace","not_placed",@{$data_point}{qw(chr start end label)});;
		  goto SKIPDATUM;
	      } else {
		  printdebug_group("textplace","placed",@{$data_point}{qw(chr start end label)});
	      }
	      ...
	      my $fontname = get_file_annotation( file => $fontfile );
		  ...
		  [
		   ...
		   getxypos($astart,
			    $data_point->{radius_new} + sum( @link_dims[ 0, 1 ] )
		   ),
		  ],
		  $link_thickness,
		  $line_colors
		  ...
		  [
		   ...
		   getxypos($aend,
			    $data_point->{radius_new} + sum( @link_dims[ 0, 1, 2 ] )
		   ),
		  ],
		  $link_thickness,
		  $line_colors
		  ...
		  [
		   getxypos($aend,
		       ...
		       getxypos($aend,
				$data_point->{radius_new} + sum( @link_dims[ 0, 1, 2, 3 ] )
		   ),
		  ],
		  $link_thickness,
		  $line_colors
	  ...
	  if ( !$value_outofbounds ) {
		...
		if ( seek_parameter("stroke_color", $datum, @param_path ) ) {
		    ...
		    $svg_colors .= "fill:none;";
			  ...
			  unit_strip(seek_parameter( "glyph_size", $datum, @param_path ))/2,
			  unit_strip(seek_parameter( "stroke_thickness", $datum, @param_path )) || 0, 
			  $svg_colors,
			  );
		printsvg($svg);
	      }
		...
		if ( seek_parameter( "fill_color", $datum, @param_path ) ) {
		    $IM->filledArc(
			getxypos( $angle, $radius ),
			unit_strip(seek_parameter("glyph_size", $datum, @param_path),"p"),
			unit_strip(seek_parameter("glyph_size", $datum, @param_path),"p"),
			0, 360,
		  ...
		  if ( seek_parameter( "stroke_thickness", $datum, @param_path ) ) {
		      my $thickness = unit_strip(seek_parameter( "stroke_thickness", $datum, @param_path ));
		...
		report_image_map(shape=>"poly",
		  ...
		  my $stroke_thickness = "";
		  if(seek_parameter("stroke_color", $datum, @param_path)) {
		      $stroke_color = sprintf(" stroke: rgb(%d,%d,%d); ",
					      rgb_color(seek_parameter("stroke_color", $datum, @param_path)));
		  }
		  if(my $st = seek_parameter("stroke_thickness", $datum, @param_path)) {
		      $stroke_thickness = sprintf(" stroke-width: %.1f; ", $st);
		  }
		  my $svg = sprintf(
		      q{<polygon points="%s" style="%s%sfill: rgb(%d,%d,%d);" />},
		      join( $SPACE, map { join( $COMMA, @$_ ) } $poly->vertices ),
		      $stroke_thickness,
		      $stroke_color,
		      rgb_color(seek_parameter("fill_color", $datum, @param_path  ) ),
		      );
		...
		if($thickness == 1 && rgb_color_opacity($stroke_color) == 1) {
		  # this is a 1-px thick line and the color has no transparency - 
		  # go ahead and antialias this line
		    $IM->setAntiAliased(fetch_color($stroke_color,$COLORS));
		  $color_obj = gdAntiAliased;
		} else {
		  $IM->setThickness($thickness) if $thickness > 1;
		  $color_obj = fetch_color($stroke_color,$COLORS);
		}
	    ...
	    start_timer("track_line_histogram");
	  my $url = seek_parameter("url",$data_point,$datum,@param_path);
	  $url = format_url(url=>$url,param_path=>[$data_point,$datum,@param_path]);
	  #
	  ...
	  if (   $data_point_prev
		...
		#printinfo($d,$max_gap_value,$gap_pass);
	      } else {
		my $d =
		  sqrt( ( $xp - $x )**2 + ( $yp - $y )**2 );
		$gap_pass = $d <= $max_gap_value;
	  ...
	  my $thickness = seek_parameter( "thickness", $datum, @param_path ) || seek_parameter( "stroke_thickness", $datum, @param_path );
	  $thickness    = unit_strip($thickness,"p");
	  my ($line_brush, $line_colors);
	  if($thickness) {
	      ( $line_brush, $line_colors ) = fetch_brush( $thickness, $thickness );
	  }
	  ...
	  if ( $plot_type eq "line" ) {
	    ...
	    goto SKIPDATUM if is_num_notequal($data_point->{ideogram_idx}, $data_point_next->{ideogram_idx});
	    goto SKIPDATUM if is_num_notequal($data_point->{ideogram_idx}, $data_point_prev->{ideogram_idx});
	    ...
	    if (  !$data_point_prev || is_num_notequal($data_point_prev->{ideogram_idx},$data_point->{ideogram_idx} )) {
		$first_on_ideogram = 1;
	    }
	    if ( !defined $data_point_next || is_num_notequal($data_point->{ideogram_idx},$data_point_next->{ideogram_idx})) {
		$last_on_ideogram = 1;
	    }
	    ...
	    if (
		$data_point_prev && $gap_pass && 
		is_num_equal($data_point_prev->{ideogram_idx},$data_point->{ideogram_idx})
		&& (
	    ...
	    stop_timer("track_line_histogram");
	} elsif ( $plot_type eq "tile" ) {
	    start_timer("track_tile");
	    my $set;
	    eval { $set = Set::IntSpan->new(sprintf( "%d-%d", $data_point->{start}, $data_point->{end} ) ); };
	    ...
	    my ($freelayer) = grep( !$_->{set}->intersect($padded_set)->cardinality,
	  ...
	  my ( $line_brush, $line_colors );
	  my $thickness = seek_parameter("stroke_thickness", $datum, @param_path) || seek_parameter("thickness", $datum, @param_path);
	  if($thickness) {
	      ($line_brush,$line_colors) = fetch_brush($thickness,$thickness,
...
sub fetch_brush {
  ...
  if ( defined $color && $w && $h ) {
      ...
      # with circle
      $brush->arc($w/2, $h/2, $w0, $h0, 0, 360, fetch_color($color,$brush_colors));
  }
...
sub eval_expression {
  ...
  while ( $condition =~ /(_(\w+)_)/ ) {
	...
	if ( $datum->{data}[$varnum] ) {
	  ...
	  fatal_error("rules","no_such_field",$condition,$string,$varroot);
	}
      } else {
	printdumper($datum->{data});
	fatal_error("rules","wrong_num_elements",$condition,$string,$varnum);
      }
    } else {
	...
	#replace_string(\$condition,$string,"undef");
	  fatal_error("rules","no_field_value",$condition,$string);
      }
    }
  }
  ...
  fatal_error("rules","parse_error",$condition,$@) if $@;
...
sub test_rule {
  ...
  printdebug_group("rule","rule condition",$condition);
...
sub draw_bezier {
  ...
  return if !$PNG_MAKE;
  if ( $thickness > 100 ) {
      fatal_error("links","too_thick",$thickness);
  } elsif ( $thickness < 1 ) {
      fatal_error("links","too_thin",$thickness);
  }
  ...
  my $line_color_obj;
  printdebug_group("link","thickness",$thickness,"color",$color);
  if($thickness == 1) {
      if(rgb_color_opacity($color) == 1) {
	  # this is a 1-px thick line and the color has no transparency - 
	  # go ahead and antialias this line
	  $IM->setAntiAliased(fetch_color($color,$COLORS));
	  $line_color_obj = gdAntiAliased;
      } else {
	  $line_color_obj = fetch_color($color,$COLORS);
      }
  } elsif($thickness > 1) {
      if(rgb_color_opacity($color)==1) {
	  printdebug_group("link","link brush");
	  my ($brush,$brushc) = fetch_brush($thickness,$thickness,$color);
	  $IM->setBrush($brush);
	  $line_color_obj = gdBrushed;
      } else {
	  $IM->setThickness($thickness) if $thickness > 1;
	  $line_color_obj = fetch_color($color,$COLORS);
	  printdebug_group("link","link color",$line_color_obj);
      }
  }
...
sub aa_color {
    ...
    my $imc = fetch_color($color,$imcolors);
    if(rgb_color_opacity($color) == 1) {
	$im->setAntiAliased( $imc );
...
sub draw_highlights {
  ...
  for my $highlight_set ( make_list( $datasets->{dataset} ) ) {
	...
	if ($test) {
		...
		my $value = seek_parameter( $param, $data, $highlight_set, $datasets ) || 0;
...
sub newspan {
  ...
  if($start == $end || ! defined $end) {
    return Set::IntSpan->new(sprintf("%.0f",$start));
      ...
      fatal_error("data","malformed_span",$start,$end);
  } else {
...
sub parse_parameters {
  ...
  for my $key ( keys %$node ) {
    ...
    if (
	defined_and_zero($CONF{restrict_parameter_names}) || 
    ...
    if(not_defined_or_one($CONF{restrict_parameter_names})) {
	fatal_error("configuration","unsupported_parameter",$key,$type) unless $continue;
    }
  }
...
sub read_data_file {
  ...
  LINE:
	...
	next if /^\s*$/;
	s/\s+\#\s+.*//;
	my $delim_rx = $CONF{file_delim} || undef;
	if($delim_rx && $CONF{file_delim_collapse}) {
	    $delim_rx .= "+";
	}
	...
	for my $i ( 0 .. @{ $fields->{$type} } - 1 ) {
	    ...
	    if ( $field eq "options" && defined $value) {
	...
	if (   defined $datum->{data}{start}
		...
		fatal_error("track","start_larger_than_end",$file,$type,$datum->{data}{start},$datum->{data}{end});
	    }
	}
	...
	if ($datum) {
		...
		#
		if ( defined $datum->{data}{value} && $datum->{data}{value} =~ /,/ ) {
  ...
  # defined (rspacing*ideogram_size) by process_tick_structure()
	...
	#
	  ...
	  if ( $CONF{show_tick_labels}
	      ...
	      my $tickfontname = get_file_annotation( file => $tickfont);
...
sub read_chromosomes_order {
  ...
  if ( $CONF{chromosomes_order} ) {
      ...
      open(CHRORDER, $CONF{chromosomes_order_file}) || fatal_error("io","cannot_read",$CONF{chromosomes_order_file},"chromosome order",$!);
      while (<CHRORDER>) {
	  chomp;
	  my ($tag) = split;
	  push( @chrorder, $tag );
...
sub draw_text {
	...
	%params = validate(
		...
		pangle        => 0,
		radius        => 0,
		forcerotation => { default  => 0 },
		text          => 1,
		xy            => { type => ARRAYREF },
		svgxy         => { optional => 1, type => ARRAYREF },
		svgangle      => { default => 0 },
		svganchor     => { default => "end" },
		chr           => 1,
		start         => 1,
		fontkey       => 0,
		end           => 1,
	...
	%params = @_;
	$params{forcerotation} ||= 0;
	$params{svgangle}      ||= 0;
	$params{svganchor}     ||= "end";
	$params{color}         ||= "black";
    }
    
    my ($w,$h);
    if(defined $params{width} && defined $params{height}) {
	($w,$h) = @params{qw(width height)};
    } else {
	my ($w, $h) = get_label_size(font_file => $params{font},
				     size      => $params{size},
				     text      => $params{text});
    }
	...
	my @bounds = $IM->stringFT(
	    fetch_color($params{color},$COLORS),
	    @params{qw(font size)},
	    ($params{angle}||0) + ($params{forcerotation}||0) * $DEG2RAD,
	    @{ $params{xy} },
	    $params{text});
	...
	my $tanchor = "start";
	printdebug_group("svg", 
			 "svglabel", $params{text}, 
	...
	my $font_family = "";
	if($params{fontname}) {
	    $font_family = qq{font-family="'$params{fontname}'"};
	}
	my $svg = sprintf(
	    qq{<text x="%.1f" y="%.1f" %s style="fill: rgb(%d,%d,%d); font-size: %.1fpx; text-anchor: %s" transform="rotate(%.1f,%.1f,%.1f)">%s</text>},
	    @{ $params{svgxy} },
	    $font_family,
	    rgb_color( $params{color} ),
	    $CONF{svg_font_scale} * $params{size},
	    $tanchor,
	    $params{svgangle}||0 + $params{forcerotation}||0,
	    @{ $params{svgxy} },
	    $svg_text||$EMPTY_STR,
	    );
	#printdumper(\%params);
...
sub read_plotdata {
  ...
  my $file = shift;
  my %data;
  open( D, $file ) || fatal_error("io","cannot_read",$file,"track data",$!);
  while (<D>) {
    chomp;
    next if /^\s*\#/;
  ...
  sub slice {
	...
	defined $params{fillcolor};
	  ...
	  $svg = sprintf(
	      ...
	      $params{edgestroke} ? sprintf( "stroke-width: %.1f;", unit_strip($params{edgestroke}) ) : "stroke: none;",
	      
	      $params{edgestroke} && $params{edgecolor} && rgb_color_opacity($params{edgestroke})
	      ?
	      sprintf("stroke-opacity: %f;", rgb_color_opacity($params{edgecolor})) : $EMPTY_STR,
	      
	      $params{edgestroke} && $params{edgecolor} ? sprintf( "stroke: rgb(%d,%d,%d);",
	  ...
	  $svg = sprintf(
	      ...
	      $params{edgestroke} && $params{edgecolor} && rgb_color_opacity($params{edgecolor}) ?
	      sprintf("stroke-opacity: %f;", rgb_color_opacity($params{edgecolor})) : $EMPTY_STR,
	      
	      $params{edgestroke} && $params{edgecolor} ? 
	    sprintf( "stroke: rgb(%d,%d,%d);", rgb_color( $params{edgecolor} ) ) : $EMPTY_STR,
	    );
    } else {
	...
	$svg = sprintf(
	    ...
	    sprintf( "stroke: rgb(%d,%d,%d);", rgb_color( $params{edgecolor} ) ) : $EMPTY_STR,
	    # style 3
	    $params{edgecolor} && rgb_color_opacity( $params{edgecolor} ) < 1
	    ? sprintf( "stroke-opacity: %.3f;", rgb_color_opacity( $params{edgecolor} ) ) : $EMPTY_STR,
	    # style 4
	    $params{fillcolor} ? sprintf( "fill: rgb(%d,%d,%d);", rgb_color( $params{fillcolor} ) ) : 'fill: none;',
	    # style 5
	    $params{fillcolor} && rgb_color_opacity( $params{fillcolor} ) < 1 ? 
	    sprintf( "opacity: %.3f;", rgb_color_opacity( $params{fillcolor} ) )
	    : $EMPTY_STR,
	    );
    }
	...
	my $thickness = unit_strip($params{edgestroke});
	my $color     = $params{edgecolor} || $params{fillcolor} || $CONF{default_color} || "black";
	my $line_color_obj;
	if($thickness == 1 && rgb_color_opacity($color) == 1) {
	    # this is a 1-px thick line and the color has no transparency - 
	    # go ahead and antialias this line
...
sub getanglepos {
  ...
  # v0.56-pre2 - use mod instead of v0.40 fix above
...
sub read_karyotype_file {
	...
	if ( $field =~ /chr/ ) {
	    ...
	    fatal_error("data","unknown_karyotype_line",$field);
...
sub validate_karyotype {
  ...
  for my $chr ( keys %$karyotype ) {
	...
	fatal_error("band_on_missing_chr",$chr);
    }
	...
	if ( $band->{set}->diff($chrset)->cardinality ) {
	  ...
	  fatal_error("data","band_overlaps",$band->{name},$chr,$max_band_overlap);
      }
...
sub old___fetch_color {
    my ($color_name,$color_table) = shift;
    $color_table ||= $COLORS;
    if(exists $COLORS->{$color_name}) {
	return $COLORS->{$color_name};
    } elsif (my $lc = $COLORS->{lc $color_name}) {
	printwarning("Circos colors should be lowercase. You have asked for color [$color_name] and it was interpreted as [$lc]");
	return $lc;
    } else {
