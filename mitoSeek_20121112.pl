...
if ($inbam2) {
	...
	my %hash=_determine_heteroplasmy($mitobasecall1,$hp,$ha,$isall,$sb,$mitoheteroplasmy1);
}
...
# Structure variation
if($inbam2){
	_info($step++.".1) Structure variation detection from $mitobam1 (Output: $mitostructure1)");
	_structure_variants($inbam1,$isbam,$mmq,$regionbed,$str,$mitostructure1);
	_info($step.".2) Structure variation detection from $mitobam2 (Output: $mitostructure2)");
	_structure_variants($inbam2,$isbam,$mmq,$regionbed,$str,$mitostructure2);
}else{
	_info($step++.") Structure variation detection from $mitobam1 (Output: $mitostructure1)");
	_structure_variants($inbam1,$isbam,$mmq,$regionbed,$str,$mitostructure1);
}
...
sub _structure_variants{
	...
	my %temphash;
	if ($isbam) {
		open( IN, "$samtools view -L $regionbed -q $mmq $inmitobam |" ) or die $!;
	}
	else {
		open( IN, "$samtools view -S -L $regionbed -q $mmq $inmitobam |" ) or die $!;
	}
	open(OUT,">$outfile") or die $!;
	print OUT join "\t",("#MitoChr","MitoPos","VarChr","VarPos","SupportedReads","MeanMappingQuality\n");
	while(<IN>){
		my (
	...
	foreach my $k (sort keys %temphash){
		if($temphash{$k}->{count}>=$str){	
			print OUT join "\t",($k,$temphash{$k}->{'count'},_mymean($temphash{$k}->{'mapping'}));
			print OUT "\n";
		}
	}
	close OUT;
...
#	}
	...
	print OUT join "\t",("#chr","pos","ref","forward_A","forward_T","forward_C","forward_G","reverse_A","reverse_T","reverse_C","reverse_G",
						"heteroplasmy","95\%ci_lower","95\%ci_upper","major_allele","minor_allele","major_allele_count","minor_allele_count");
	if($sb){
	...
	foreach my $loc (sort {$a<=>$b} keys %result){
		if($result{$loc}->{'stat'}==2){
			print OUT join "\t",($result{$loc}->{'chr'},$loc,$result{$loc}->{'ref'},$result{$loc}->{'A'},$result{$loc}->{'T'},$result{$loc}->{'C'},$result{$loc}->{'G'},
					$result{$loc}->{'a'},$result{$loc}->{'t'},$result{$loc}->{'c'},$result{$loc}->{'g'},
					$result{$loc}->{'heteroplasmy'},$result{$loc}->{'lower'},$result{$loc}->{'upper'},$result{$loc}->{'major_allele'},$result{$loc}->{'minor_allele'},
					$result{$loc}->{'major_allele_count'},$result{$loc}->{'minor_allele_count'});
			if($sb>0){
...
#No matter the minor_allele_count meet the cutoff of not, it is already a somatic mutation
	   ...
	   #Only the minor_allele_count meet the cutoff and it is a somatic mutation
		...
		print OUT join "\t",("#chr","pos","ref","tumorGenotype","tumorDepth","normalGenotype","normalDepth\n");
		if ($issomatic) {
